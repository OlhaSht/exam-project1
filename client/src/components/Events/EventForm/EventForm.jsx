...
const EventForm = ({ setTasks }) => {
  const validationSchema = Yup.object().shape({
    ...
    eventDate: Yup.string()
      ...
      .test('isFutureTime', 'Time must be in the future', function () {
        ...
        const eventDateTime = new Date(
          `${eventDate}T${eventTimeHours}:${eventTimeMinutes}:00`
        );
    ...
    notifyDate: Yup.string()
      ...
      .test(
        'isValidNotifyDate',
        'Notify must be in the future and before event',
        function (value) {
          const {
            notifyTimeHours,
            notifyTimeMinutes,
          ...
          const notifyDateTime = new Date(
            `${value}T${notifyTimeHours}:${notifyTimeMinutes}:00`
          );
          const now = new Date();

          if (notifyDateTime <= now) {
            return this.createError({
              message: 'Notify time must be in the future',
            });
          }

          if (notifyDateTime > eventDateTime) {
            return this.createError({
              message: 'Notify must be before event time',
            });
          }
